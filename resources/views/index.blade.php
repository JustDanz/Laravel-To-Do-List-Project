...
<div class="container py-5">
    ...
    <!-- Trello Board -->
    <div class="row mb-5">
        @foreach (['belum_mulai' => 'Belum Mulai', 'proses' => 'Proses', 'selesai' => 'Selesai'] as $key => $label)
        <div class="col-md-4 mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <span class="badge bg-light text-dark">{{ count($tasks[$key] ?? []) }}</span>
            </div>
            <div class="task-column" data-status="{{ $key }}" id="column-{{ $key }}">
                @foreach($tasks[$key] ?? [] as $task)
                    <div class="task-card mb-3" data-id="{{ $task->id }}" data-status="{{ $key }}">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <strong>{{ $task->title }}</strong>
                            <small class="text-muted">{{ $task->day_of_week }}</small>
                        </div>
                        @if($task->description)
                        <p class="text-muted mb-3 small">{{ $task->description }}</p>
                        @endif
                                ...
                                <form action="{{ route('tasks.update', $task->id) }}" method="POST">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select name="day_of_week" class="form-select" required>
                                                @foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'] as $day)
                                                    <option value="{{ $day }}" {{ $task->day_of_week == $day ? 'selected' : '' }}>{{ $day }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="mb-3">
    ...
    <div class="task-table">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <span class="badge bg-primary">
                Total: {{ array_sum(array_map('count', $tasks)) }}
            </span>
        </div>
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    @php $no = 1; @endphp
                    @foreach(['belum_mulai', 'proses', 'selesai'] as $status)
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-{{ 
                                    ...
                                    <i class="fas fa-{{ 
                                        $task->status == 'selesai' ? 'check-circle' : 
                                        ($task->status == 'proses' ? 'spinner' : 'clock') 
                                    }} me-1"></i>
                                    {{ ucfirst(str_replace('_', ' ', $task->status)) }}
                                </span>
                            </td>
                            <td>
                                <button class="btn btn-sm btn-outline-primary me-1" data-bs-toggle="modal" data-bs-target="#editModal{{ $task->id }}">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <form action="{{ route('tasks.destroy', $task->id) }}" method="POST" class="d-inline">
                                    @csrf
